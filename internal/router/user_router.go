...
func setupUserRoutes(
	...
	api.Use(middleware.SetupCheck())
	{
		...
		publicAuth.Use(middleware.RateLimit())
		{
			publicAuth.POST("/register/captcha", authH.RegisterCaptcha)
			publicAuth.POST("/register", authH.Register)
			publicAuth.POST("/login/code", authH.LoginCode)
			publicAuth.POST("/login", authH.UserLogin)
			publicAuth.GET("/oauth/:provider/login", authH.OAuthLogin)
			publicAuth.GET("/oauth/:provider/callback", authH.OAuthCallback)
			publicAuth.POST("/oauth/:provider/register", authH.OAuthRegister)
			publicAuth.GET("/oauth/:provider/bind/callback", authH.OAuthBindCallback)
			publicAuth.GET("/oidc/:provider/login", authH.OIDCLogin)
			publicAuth.GET("/oidc/:provider/callback", authH.OIDCCallback)
			publicAuth.POST("/oidc/:provider/register", authH.OIDCRegister)
			publicAuth.GET("/oidc/:provider/bind/callback", authH.OIDCBindCallback)
		}
