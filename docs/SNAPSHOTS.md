...
当前代码已在 `logs` 中基础落地 `request_id`、`error_code`、`error_source`、`upstream_status`、`usage_source`、基础 `route_snapshot` 和基础 `billing_snapshot`。`route_snapshot` 目前覆盖请求模型、候选数量、选中通道、provider、分组、优先级、权重、模型重写摘要和非流式重试摘要；候选过滤原因仍需继续补齐。`billing_snapshot` 目前覆盖结算状态、usage_source、Key 预算前后、用户余额前后和最终扣费；价格表达式版本和倍率仍需继续补齐。`usage_source` 目前覆盖 `upstream` 与 `minimum`，`adapter`、`tokenizer` 和 `estimate` 需要随对应能力实现后再写入。
