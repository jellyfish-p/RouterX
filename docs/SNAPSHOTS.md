...
`access_rule_snapshot` 记录访问控制事实，`rate_limit_snapshot` 记录限流事实，`breaker_snapshot` 记录自动熔断过滤事实。限流和熔断快照虽然嵌在 `policy_snapshot` 内，也必须带 `request_id` 和 `created_at`，便于从单次拒绝直接回溯入口日志。
