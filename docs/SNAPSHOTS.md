...
| `kind` | `request`、`policy`、`access_rule`、`route`、`usage`、`billing`、`rate_limit`、`breaker`、`error`、`settings`、`audit`。 |
...
| 字段 | 说明 |
|------|------|
| `request_id` | 串联入口日志、调用日志和本次请求上下文。 |
| `created_at` | 本次请求快照创建时间，使用 RFC3339 格式。 |
| `user_id` | 调用所属用户 ID。 |
...
| 字段 | 说明 |
|------|------|
| `request_id` | 串联入口日志、调用日志和本次策略判断。 |
| `created_at` | 本次策略快照创建时间，使用 RFC3339 格式。 |
...
| 字段 | 说明 |
|------|------|
| `request_id` | 串联入口日志、调用日志和本次路由选择事实。 |
| `created_at` | 本次路由快照创建时间，使用 RFC3339 格式。 |
...
| 字段 | 说明 |
|------|------|
| `request_id` | 串联入口日志、调用日志和本次 usage 来源事实。 |
| `created_at` | 本次 usage 快照创建时间，使用 RFC3339 格式。 |
...
`access_rule_snapshot` 记录访问控制事实，`rate_limit_snapshot` 记录限流事实，`breaker_snapshot` 记录自动熔断过滤事实。限流和熔断快照虽然嵌在 `policy_snapshot` 内，也必须带 `schema`、`kind`、`stage`、`source`、`redacted`、`request_id` 和 `created_at`，便于从单次拒绝直接回溯入口日志。

| 字段 | 说明 |
|------|------|
| `request_id` | 串联入口日志、调用日志和访问控制事实。 |
| `created_at` | 本次访问控制快照创建时间，使用 RFC3339 格式。 |
| `user_group` | 用户分组。 |
| `channel_group` | 通道分组。 |
| `model_access` | 模型访问判断。 |
...
| 字段 | 说明 |
|------|------|
| `request_id` | 串联入口日志、调用日志和错误响应。 |
| `created_at` | 本次错误快照创建时间，使用 RFC3339 格式。 |
...
| 路由 | 候选过滤、API Key/channel-group scope、模型重写和最终通道可复盘。 |
| 策略 | 访问拒绝、scope 收窄、限流和余额不足能解释且不调用上游；访问控制事实进入 `access_rule_snapshot`。 |
...
| 错误 | 失败日志能通过 `error_snapshot` 判断 source、是否调用上游、是否扣费、是否适合重试。 |
