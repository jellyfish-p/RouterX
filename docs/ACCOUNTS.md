...
- `users.username` 在数据库层可以为空，便于支持邮箱、手机号、OAuth 和 OIDC 等非用户名主身份；业务层创建本地用户名账户时必须写入用户名。
...
| 调整自己的用户余额 | 否 | 通过管理接口调整普通用户 | 可调整并审计 |
| 调整自己的 API Key 剩余可消耗额度或无限额度 | 是 | 是 | 是 |
| 管理普通用户 | 否 | 是 | 是 |
| 管理通道 | 否 | 是 | 是 |
| 查看全局日志和看板 | 否 | 是 | 是 |
| 修改 settings | 否 | 否 | 是 |
...
- 支持创建、禁用、删除、过期时间、剩余可消耗额度、无限额度标记和泄露风控自动禁用开关。
- 明文只返回一次。
- 鉴权成功后记录最近使用时间。
- 删除或禁用后立即清理 Redis 缓存。
