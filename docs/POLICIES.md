...
- API Key scope 已支持 `allow_models` 模型 allow-list，未命中时在上游调用前返回 `model_not_allowed` 并写失败日志。
...
- 通道已具备 `channel_group` 字段，但完整的用户分组访问控制、API Key 通道/API 类型/IP/预算 scope 和策略快照仍属于目标增强。
...
| API Key scope | `tokens.scope_json` | P1/P2 | 当前支持模型 allow-list；后续进一步收窄通道分组、协议入口、请求类型、IP 和预算。 |
...
| `api_types` | `["openai.chat", "openai.models"]` | 只允许这些接口能力。 |
...
- API Key scope 已支持模型收窄；通道分组、API 类型和请求类型收窄仍需补齐。
...
- 支持 IP/CIDR、日预算、月预算、RPM、TPM、并发限制和异常告警。
...
| API Key scope 收窄 | 允许模型成功，未允许模型拒绝且不调用上游。 |
