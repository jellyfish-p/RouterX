...
| API Key scope | `tokens.scope_json` | P1/P2 | 当前支持模型 allow-list、APIType allow-list、通道分组 allow-list、入口协议 allow-list、IP/CIDR allow-list、方法路径 allow-list、日预算、月预算和并发上限。 |
...
- API Key scope 已支持模型、APIType、通道分组、入口协议、IP/CIDR、方法路径、日预算、月预算和并发上限收窄。
...
- 已支持入口协议 allow-list、IP/CIDR allow-list、日预算、月预算和并发上限；RPM、TPM 和异常告警仍待补。
...
| API Key scope 收窄 | 允许模型/APIType/通道分组/入口协议/IP/方法路径、未达日/月预算且未超并发上限的请求成功；未允许模型、APIType、通道分组、入口协议、IP、方法路径、达到日/月预算或达到并发上限时拒绝且不调用上游。 |
