...
| API Key scope | `tokens.scope_json` 目标字段 | P1/P2 | 只能进一步收窄模型、通道分组、协议入口、请求类型、IP 和预算。 |
...
| scope | 示例 | 语义 |
|-------|------|------|
| `models` | `["gpt-4o-mini", "claude-3-5-sonnet"]` | 只允许这些调用方模型名。 |
...
- API Key scope 支持模型、通道分组、API 类型和请求类型收窄。
