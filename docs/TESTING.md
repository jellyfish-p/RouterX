...
| P1 | 通道候选缓存 | 已覆盖进程内缓存命中、Redis 共享候选快照、`routing.channel_cache.preload` 启动预热/关闭 no-op/通道变更后预热、`routing.channel_cache.version` 变化后回源、默认 settings 和非法配置校验；继续补主动 pub/sub 广播失效 |
