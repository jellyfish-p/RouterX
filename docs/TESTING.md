...
| P2 | 支付充值 | 充值码批次/备注/过期策略、Stripe Checkout Session 创建、Stripe/易支付签名、金额校验、订单状态、重复回调幂等、额度流水、webhook 入账审计、Stripe 全额/部分退款和扣回审计、Stripe 争议事件和可选 API Key 禁用审计、支付人工补账/扣回审计、支付人工退款落账审计；完整争议生命周期和 provider 侧自动发起退款流程待补 |
