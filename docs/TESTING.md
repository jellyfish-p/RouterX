...
| `TestChatCompletionSuccessLogsAndDeductsQuota` | Chat 非流式成功调用、上游 usage_source、基础 route_snapshot、含 P0 表达式/默认倍率/预算前后摘要的基础 billing_snapshot、日志、用户额度、Key 预算和账单聚合 |
...
| P1 | 调用事实快照 | 调用日志已覆盖 request_id、error_code、error_source、upstream_status、基础 usage_source、含过滤/模型重写/重试摘要的基础 route_snapshot 和含 P0 表达式/默认倍率/预算前后摘要的基础 billing_snapshot；继续补 request、policy、完整 route、usage、完整 billing、error 快照脱敏和历史解释 |
