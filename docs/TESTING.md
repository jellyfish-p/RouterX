...
| 禁用通道 | 502，下游计数 0 |
| 模型不匹配 | 502，下游计数 0 |
| API Key scope 未允许模型或 APIType | 403，下游计数 0 |
...
| P2 | 高级 API Key 管理 | 基础生命周期审计、轮换、泄露上报、单 Key 用量摘要、管理员跨用户查询、批量禁用、模型 allow-list scope 和 APIType allow-list scope 拒绝已覆盖；通道分组/IP/预算 scope、持久化来源摘要、批量过期、风险视图和缓存失效待补 |
