...
- P0 优先保证 OpenAI-compatible Chat/Models 非流式可用闭环。
...
- OpenAI-compatible Chat Completions 非流式和 Models 已形成基础闭环。
...
- `stream=true` 当前 P0 明确返回 `unsupported_stream`，不会调用下游。
...
| OpenAI-compatible Chat | `POST /v1/chat/completions` | 基础实现，目标 P0 稳定 | P0 | P0 只承诺非流式；`stream=true` 返回明确错误。 |
...
| 流式 | 明确拒绝 `stream=true` | SSE/chunk 转换、客户端断开取消、流式 usage | 高级流式事件、部分失败和审计摘要 |
...
- 不调用下游。
- 不扣费。
- 可以写失败日志，日志中标注入口协议、APIType 和请求模型。
