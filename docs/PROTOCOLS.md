...
- Anthropic count_tokens 和 Gemini countTokens 当前是本地近似计数能力，不等同于真实厂商 tokenizer 完全一致；Gemini countTokens 会优先按 `contents`/`systemInstruction`/`generateContentRequest` 的实际 prompt 文本计数，而不是整段 JSON 外壳。
