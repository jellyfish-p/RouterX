...
- `?format=gemini|google` 返回 Gemini 模型列表外形，`?format=anthropic|claude` 返回 Anthropic 模型列表外形，`?format=openai|openai-compatible` 返回 OpenAI 外形。
...
- Gemini 模型列表的 `supportedGenerationMethods` 当前声明 `generateContent`、`streamGenerateContent`、`countTokens`、`embedContent` 和 `batchEmbedContents`，与已注册 Gemini 入口保持一致。
