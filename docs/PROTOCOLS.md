...
| OpenAI Responses | `POST /v1/responses` | 基础实现 | P1 | OpenAI-compatible 通道可 JSON 透传；Azure OpenAI 通道可 JSON 转发到 `/openai/v1/responses?api-version=preview`，保留 `model` 作为 deployment 名；`input_tokens/output_tokens/total_tokens` usage 已映射到日志和扣费；完整 Responses 原生字段矩阵和流式 Responses 仍需补齐。 |
