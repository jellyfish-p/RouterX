...
- OpenAI-compatible Chat Completions 非流式、Chat/Legacy Completions 基础 SSE 流式、Responses 基础 JSON 透传和 Models 已形成基础闭环。
...
- OpenAI Chat 和 Legacy Completions 的 `stream=true` 仅对 OpenAI SSE 形态通道开放；非 OpenAI SSE 通道返回 `unsupported_stream_channel` 且不会调用下游。
...
| 流式 | OpenAI Chat/Legacy Completions 基础 SSE 可用，未支持协议明确拒绝 | SSE/chunk 转换、客户端断开取消、流式 usage | 高级流式事件、部分失败和审计摘要 |
