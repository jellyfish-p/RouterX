...
- Anthropic Messages 入口已有基础转换链路：Anthropic Messages 转内部 OpenAI Chat 形态，再返回 Anthropic 兼容响应。
- Gemini generateContent 入口已有基础转换链路：Gemini contents 转内部 OpenAI Chat 形态，再返回 Gemini 兼容响应。
...
| Anthropic Messages | `POST /v1/messages` | 基础实现 | P1 | 已有基础转换，仍需补字段保真、流式、错误外形和测试矩阵。 |
...
| Gemini generateContent | `POST /v1/models/{model}:generateContent` | 基础实现 | P1 | 已有基础转换，仍需补字段保真、错误外形和测试矩阵。 |
