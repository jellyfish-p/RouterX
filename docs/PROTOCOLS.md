...
- Azure OpenAI adapter 已支持 Chat Completions、Completions 和 Embeddings 基础转发，使用 deployment 路径、`api-version` query 和 `api-key` header；Image Generations 和 Audio Speech 支持 Azure `/openai/v1` JSON 转发并保留 `model` 作为 deployment 名；Audio Transcriptions/Translations 支持 Azure `/openai/v1` multipart 转发、`api-key` 和最低计费；管理端模型拉取会读取 `/openai/deployments` 并返回 deployment id；其他 Azure API 仍属于目标扩展，不能承诺生产兼容。
...
| Azure OpenAI | 基础实现（Chat/Completions/Embeddings/Image Generations/Audio） | P1/P2 | Chat Completions、Completions、Embeddings 已支持 deployment 路径、`api-version`、`api-key` header、模型到 deployment 映射和 deployment 列表拉取；Image Generations、Audio Speech、Audio Transcriptions 和 Audio Translations 已支持 Azure `/openai/v1`、`api-version=preview`、`api-key` 和最低计费；其他 API 仍需补齐。 |
...
| OpenAI-compatible | Azure OpenAI | 基础实现（Chat/Completions/Embeddings/Image Generations/Audio） | Chat Completions、Completions 和 Embeddings 已支持 model 到 deployment、query `api-version`、header `api-key` 和 OpenAI usage 扣费；Image Generations、Audio Speech、Audio Transcriptions 和 Audio Translations 已支持 Azure `/openai/v1` 形态并按无 usage 最低计费；Image Edits/Variations 等仍需补齐。 |
