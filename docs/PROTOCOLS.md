...
- Azure OpenAI adapter 已支持 Chat Completions 和 Embeddings 基础转发，使用 deployment 路径、`api-version` query 和 `api-key` header；管理端模型拉取会读取 `/openai/deployments` 并返回 deployment id；其他 Azure API 仍属于目标扩展，不能承诺生产兼容。
...
| Azure OpenAI | 基础实现（Chat/Embeddings） | P1/P2 | Chat Completions、Embeddings 已支持 deployment 路径、`api-version`、`api-key` header、模型到 deployment 映射和 deployment 列表拉取；其他 API 仍需补齐。 |
...
| OpenAI-compatible | Azure OpenAI | 基础实现（Chat/Embeddings） | Chat Completions 和 Embeddings 已支持 model 到 deployment、query `api-version`、header `api-key` 和 OpenAI usage 扣费；Images/Audio 等仍需补齐。 |
