...
- P0 优先保证 OpenAI-compatible Chat/Models 非流式可用闭环；当前已推进 OpenAI Chat 基础 SSE 流式。
...
- OpenAI-compatible Chat Completions 非流式、OpenAI SSE 基础流式和 Models 已形成基础闭环。
- Anthropic Messages 入口已有基础转换链路：Anthropic Messages 转内部 OpenAI Chat 形态，再返回 Anthropic 兼容响应；非文本 content blocks 会降级为 compact JSON 文本。
...
- Azure OpenAI adapter 已注册类型，但仍属于目标扩展，不能承诺生产兼容。
- OpenAI Chat `stream=true` 仅对 OpenAI SSE 形态通道开放；非 OpenAI SSE 通道返回 `unsupported_stream_channel` 且不会调用下游。
...
| OpenAI-compatible Chat | `POST /v1/chat/completions` | 基础实现，目标 P0/P1 稳定 | P0/P1 | 非流式为 P0 主链路；OpenAI SSE 基础流式已可用，客户端断开和多协议 chunk 转换仍属 P1。 |
| OpenAI Responses | `POST /v1/responses` | 已注册 | P1 | 需要补请求/响应转换、usage、错误和测试。 |
| OpenAI Completions | `POST /v1/completions` | 已注册 | P1 | 适合后续兼容旧 SDK 和旧模型。 |
| OpenAI Embeddings | `POST /v1/embeddings` | 已注册 | P1 | 需要补上游 endpoint、usage/计费和模型能力校验。 |
| OpenAI Images | `/v1/images/*` | 已注册 | P2 | 涉及 multipart、文件大小、安全扫描和成本策略。 |
| OpenAI Audio | `/v1/audio/*` | 已注册 | P2 | 涉及 multipart、流式/二进制响应和大小限制。 |
| OpenAI Moderations | `POST /v1/moderations` | 已注册 | P2 | 需要补安全策略、错误和账单边界。 |
| Anthropic Messages | `POST /v1/messages` | 基础实现 | P1 | 已有基础转换和基础字段降级测试，仍需补原生字段保真、流式、下游错误外形和完整 SDK 矩阵。 |
...
| Gemini generateContent | `POST /v1/models/{model}:generateContent` | 基础实现 | P1 | 已有基础转换和基础字段降级测试，仍需补原生字段保真、流式、下游错误外形和完整 SDK 矩阵。 |
| Gemini streamGenerateContent | `POST /v1/models/{model}:streamGenerateContent` | 已注册 | P1 | 需要完整 SSE/chunk 转换和流式 usage 策略。 |
| Gemini countTokens | `POST /v1/models/{model}:countTokens` | 基础实现 | P1 | 当前为近似计数；未来可按上游或 tokenizer 精确化。 |
| Gemini embedContent | `POST /v1/models/{model}:embedContent` | 目标扩展 | P1/P2 | 需要补路由、Adapter 和计费。 |
| Gemini batchEmbedContents | `POST /v1/models/{model}:batchEmbedContents` | 目标扩展 | P1/P2 | 需要补批量限制、计费和错误聚合。 |
...
| Azure OpenAI | 目标扩展 | P1/P2 | deployment 路径、`api-version`、`api-key` header、模型到 deployment 映射。 |
...
| OpenAI-compatible | Azure OpenAI | 目标扩展 | model 到 deployment，query `api-version`，header `api-key`，错误脱敏。 |
...
| 流式 | OpenAI Chat 基础 SSE 可用，未支持协议明确拒绝 | SSE/chunk 转换、客户端断开取消、流式 usage | 高级流式事件、部分失败和审计摘要 |
...
- `routerx.provider.<provider>` 只在选中对应上游时生效。
- 真实厂商上游发出前必须剥离 `routerx` 字段。
- RouterX-Compatible 上游可以继续接收 `routerx` 字段，但必须增加 hop 限制和链路追踪。
...
- OpenAI-compatible Chat `stream=true` 已支持 OpenAI SSE 形态通道的基础转发。
- 当前从 OpenAI SSE `data:` JSON chunk 中提取 `usage`，结束后扣费并写成功日志。
- Anthropic/Gemini 流式和非 OpenAI SSE 通道 chunk 转换仍未开放；不支持的流式入口返回 `unsupported_stream` 或 `unsupported_stream_channel`。

P1 目标：

- OpenAI SSE、Anthropic Messages Stream、Gemini streamGenerateContent 都有入口协议兼容输出。
...
- Anthropic/Gemini 非流式成功、入口鉴权错误和基础字段降级已有测试；下游错误 status 映射、流式和 SDK 行为仍需要在 P1 补齐。
...
- OpenAI-compatible Chat 基础 SSE 流式可用；未支持的流式协议或通道明确失败。
