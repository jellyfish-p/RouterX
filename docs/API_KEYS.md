...
- `tokens.scope_json` 已支持基础模型 allow-list、APIType allow-list、通道分组 allow-list、IP/CIDR allow-list、方法路径 allow-list 和日预算：用户可通过 `PUT /v0/user/token/:id/scope` 写入 `allow_models`、`api_types`、`channel_groups`、`ip_cidrs`、`methods` 与 `daily_quota`，系统在上游调用前返回 `model_not_allowed`、`token_forbidden`、`route_forbidden` 或 `insufficient_quota` 并写失败日志。
- API Key 鉴权成功后向请求上下文注入当前用户和当前 Token，供 Relay、限流、日志和计费使用。

当前代码事实是后续目标能力的基础，额度语义以 `docs/DECISIONS.md` 的 RXD-009/RXD-010 为目标口径；后续仍需补 `quota_limit`/`quota_used` 目标字段、持久化最近使用来源摘要、协议入口、月预算、并发和风险视图等更完整策略。
...
| `scope_json` | API Key 收窄策略 JSON；当前支持 `allow_models` 模型 allow-list、`api_types` APIType allow-list、`channel_groups` 通道分组 allow-list、`ip_cidrs` IP/CIDR allow-list、`methods` 方法路径 allow-list 和 `daily_quota` 日预算，后续扩展协议入口、月预算和并发限制。 |
...
P0 API Key 默认继承所属用户和系统策略。当前已支持基础模型 allow-list、APIType allow-list、通道分组 allow-list、IP/CIDR allow-list、方法路径 allow-list 与日预算 scope；后续作用域能力继续遵守只收窄、不放大的原则。
...
| PUT | `/v0/user/token/:id/scope` | 更新 `allow_models` 模型 allow-list、`api_types` APIType allow-list、`channel_groups` 通道分组 allow-list、`ip_cidrs` IP/CIDR allow-list、`methods` 方法路径 allow-list 和 `daily_quota` 日预算，并写 `api_key.scope_updated` 审计。 |
...
| 作用域扩展 | `PUT /v0/user/token/:id/scope` | 管理员或具备策略权限的用户 | 在已实现 `allow_models`、`api_types`、`channel_groups`、`ip_cidrs`、`methods` 和 `daily_quota` 基础上继续扩展协议入口、月预算和并发限制。 |
...
- 已支持基础作用域：模型 allow-list、APIType allow-list、通道分组 allow-list、IP/CIDR allow-list、方法路径 allow-list 和日预算；协议入口、月预算和并发限制仍需补齐。
...
- 已支持 IP/CIDR allow-list 和日预算；月预算、RPM/TPM、并发限制和更完整策略快照仍待补。
...
| 作用域拒绝 | 模型 allow-list 未命中时返回 `model_not_allowed`，APIType、IP/CIDR 或方法路径 allow-list 未命中时返回 `token_forbidden`，通道分组 allow-list 未命中时返回 `route_forbidden`，日预算达到上限时返回 `insufficient_quota`；都会写失败日志且不调用上游。 |
