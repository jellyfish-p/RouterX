...
- 充值码已支持用户兑换未使用码并写入额度流水；支付商品、本地 pending 订单和易支付异步通知幂等入账基础能力已可用，Stripe 插件、退款和全量支付审计仍需补齐，契约以 `docs/PAYMENTS.md` 为准。
...
| WP2-3 支付和充值 | P2 | payment products/orders/events、quota_transactions、webhook | 支付商品、本地 pending 订单和易支付异步通知幂等入账已覆盖；后续补 Stripe、退款和人工修正流水审计 | 新增 Stripe、退款和人工修正测试 |
