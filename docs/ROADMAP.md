...
RouterX 当前已经不再只是后端骨架。代码已具备可验证的 P0 开箱闭环，并扩展到多协议入口、主要上游适配、基础流式、访问控制、可靠性、独立日志库、观测审计、支付/充值/退款和高级 API 的基础链路；剩余工作按追踪矩阵作为后续增强继续管理，不作为当前不扩功能收口的阻塞。
...
- 用户、管理员、用户分组、API Key、通道、日志、设置、看板、审计、模型价格、支付、充值码和退款等接口。
- API Key 创建、列表、更新、删除、校验、轮换、泄露上报、scope 收窄、风险视图和 SHA256 哈希存储。
- 通道 CRUD、启用/禁用、连通性测试、模型拉取、密钥加密。
- 通道多 base URL、多 API Key、key 选择模式、模型重写、通道分组和扩展配置。
- `/v1` OpenAI-compatible、Gemini-compatible 和 Anthropic-compatible 入口、模型列表、非流式转发和基础 SSE 流式闭环。
- Responses、Embeddings、Images、Audio、Moderations 等高级 API 的基础透传、转换或明确拒绝路径。
- 调用日志、用户账单统计、管理看板、独立日志库、结构化日志和 Prometheus 指标基础链路。
- `/health` 和 `/ready`。

后续增强继续跟踪：

- 更完整的流式 usage fallback/估算策略、SDK 行为矩阵和长尾 provider 语义映射，能力等级以 `docs/PROTOCOLS.md` 为准。
- 系统模型价格表管理、通道模型价格覆盖、用户侧模型价格状态、成功调用后的价格表达式扣费热路径和分组倍率扣费热路径已落地；仍需补更完整访问控制快照和失败计费事实，策略契约见 `docs/POLICIES.md`。
- 充值码已支持用户兑换未使用码并写入额度流水，生成/导入/作废已写管理审计；支付商品管理、本地 pending 订单、Stripe Checkout Session 创建、Stripe/易支付 provider 退款请求、Stripe webhook 入账和明确失败审计、Stripe 全额/部分退款和扣回审计、Stripe 争议生命周期记录和可选 API Key 禁用、易支付异步通知审计、支付人工补账/扣回、支付人工退款落账和同步返回基础能力已可用，API Key 生命周期、轮换、泄露上报、用量摘要、元数据和服务账号主体过滤/脱敏导出、管理员查询/批量禁用/批量过期、用户管理、支付商品管理、settings 更新和校验拒绝、用户调额、通道管理、管理员账号管理和日志清理审计已具备基础切片，更多 provider 自动发起退款流程仍需补齐，契约以 `docs/PAYMENTS.md` 为准。
- 更多错误维度、告警动作、生产 `/ready` 细节和长期归档策略。
- 更多企业风控、多会话和账号治理增强。
...
| 阶段 | 目标 | 状态 |
|------|------|------|
| P0-1 | 初始化、账号、权限和 API Key 基础可用 | 已验证，证据见 `docs/TRACEABILITY.md` 的 P0-C1 至 P0-C6 |
| P0-2 | 通道管理和 OpenAI-compatible 非流式最小闭环 | 已验证，覆盖通道安全、模型列表、Chat 非流式、预检拒绝和错误兼容 |
| P0-3 | 日志、额度和基础账单一致 | 已验证，覆盖调用日志、用户账单聚合和基础 dashboard |
| P1-1 | 流式响应、重试、熔断和限流 | 基础链路已覆盖；更完整 usage fallback、估算策略和生产边缘场景继续跟踪 |
| P1-2 | 多协议入口和多上游转换矩阵 | 基础入口、主要上游和字段降级已覆盖；完整 SDK 行为矩阵见 `docs/PROTOCOLS.md` 与追踪矩阵 |
| P1-3 | 计费策略、访问控制、充值码和可选支付插件 | 基础闭环已覆盖；更多 provider 自动发起退款流程和长尾历史解释继续跟踪 |
| P2-1 | OAuth/OIDC 和企业账号体系 | 基础登录、绑定、补齐注册、注销恢复和验证码路径已覆盖；更多企业风控和多会话继续跟踪 |
| P2-2 | 生产观测、安全和审计增强 | 基础结构化日志、指标、审计、告警投递和密钥轮换已覆盖；更多生产观测细节继续跟踪 |
...
- 控制台和部署文档可以继续补更多引导与示例。
- 更多 SDK 示例、生产配置组合和边缘排障样例继续按 `docs/RUNBOOKS.md` 扩展。
- 新增或改变 P0 行为时，仍必须同步 `docs/ACCEPTANCE.md`、`docs/TRACEABILITY.md` 和 `docs/TESTING.md`。
...
| P0-T1b settings 注册表与就绪 | `SettingService`、setup、ready、settings cache | 初始化、修改配置、生产 readiness | current 阶段 key 存在且默认值正确；类型非法不静默降级；关键配置或迁移状态异常时生产 `/ready` 不就绪 | `TestSetupBootstrapAdminQuotaAndSettingsDefaults`、`TestSettingsValidationAndReadiness`、`TestReadinessRejectsDirtyMigrationState`、`TestReadinessRequiresEncryptionKeyForEncryptedChannelSecrets`、`TestSettingCacheRefreshesStaleRedisValues` |
...
- 路由决策快照，记录候选过滤、API Key/channel-group scope 偏好处理、最终通道、模型重写和重试结果。
...
- 指标能展示请求量、错误率、耗时、通道状态、DB/Redis/日志库状态和日志补写 outbox 积压状态。
- 支付重复通知不会重复入账。
...
- 多区域通道路由和成本优先路由。
- 模型别名、灰度路由和 A/B 路由。
- 成本分析、利润报表和更多外部告警通知；Webhook、邮件和 IM 告警基础 outbox 已落地。
...
|--------|------|----------|----------------|--------------|
| WP0-1 初始化、settings 与就绪 | P0 | setup、settings、ready、JWT | 空库可初始化；重复初始化被拒绝；初始化后 JWT 可用；settings 默认值和类型可验证；`/ready` 能反映 DB、迁移 dirty、外部数据库 Redis 依赖、JWT、密钥和关键配置状态，并能发现关键 auth/relay/rate-limit settings 配置漂移 | `TestP0BackendFlow`、`TestSetupBootstrapAdminQuotaAndSettingsDefaults`、`TestSettingsValidationAndReadiness`、`TestSettingCacheRefreshesStaleRedisValues`、`TestInitRedisSkipsEmptyConfig`、`TestReadinessRequiresRedisForExternalDatabaseMode`、`TestReadinessRejectsDirtyMigrationState`、`TestReadinessRequiresEncryptionKeyForEncryptedChannelSecrets`、`TestReadinessRejectsInvalidCriticalSettings` |
...
| WP1-6 通道候选缓存 | P1 | ChannelService、Redis、settings、POLICIES | 按模型、APIType、用户分组和通道分组预加载候选集；管理员修改后集群失效一致 | 进程内候选缓存、Redis 共享候选快照、主动 pub/sub 失效广播、启动预热、TTL/version settings、通道变更版本失效和跨实例回源已完成；`TestChannelCandidateCacheVersionTouchPublishesInvalidation` 和 `TestChannelCandidateCacheInvalidationMessageClearsLocalCache` 覆盖主动广播 |
| WP1-7 独立日志数据库 | P1 | LogService、OBSERVABILITY、BILLING、DATA_MODEL | `LOG_SQL_DSN` 初始化独立日志库，调用日志写主库事实并通过主库 outbox 补写日志库副本，管理日志列表可读日志库且查询失败回退主库事实 | `TestInitLogDBUsesConfiguredDSN`、`TestLogServiceWritesMainFactAndExternalLogDB`、`TestLogServiceFallsBackWhenExternalLogDBWriteFails`、`TestLogServiceReplaysPendingExternalLogOutbox`、`TestLogServiceWorkerReplaysPendingExternalLogOutbox`、`TestLogServiceListsFromExternalLogDBWhenConfigured`、`TestLogServiceListFallsBackToMainDBWhenExternalLogDBFails`、`TestMetricsEndpointReportsIndependentLogDBHealth` 已覆盖基础路径、outbox 异步补写、日志库健康指标和 outbox 积压指标；冷热归档仍需补齐 |
| WP2-1 企业账号 | P2 | OAuth/OIDC、captcha、session、audit | OAuth/OIDC 不因 email 自动接管；注销恢复保留历史事实；基础成功登录审计可查，企业身份登录审计继续补齐 | `TestUserLoginWritesAuditLogWithoutSecrets` 已覆盖本地密码成功登录审计；新增账号风险测试 |
| WP2-2 生产观测和审计 | P2 | structured logs、metrics、audit、ready | 成功登录、API Key 生命周期、轮换、泄露上报、元数据和服务账号主体过滤/导出、批量禁用、批量过期、基础风险视图、用户管理、支付商品管理、系统/通道模型价格管理、支付入账/明确失败/退款回调、Stripe/易支付 provider 退款请求、Stripe 争议生命周期、支付人工修正、settings 更新和校验拒绝、用户调额、充值码管理、通道管理、管理员账号管理、日志清理/导出审计、可配置 HTTP/Panic 结构化日志、调用日志 request_id/error_code/usage_source/error_source/upstream_status/基础 request_snapshot/含成功、API Key scope 拒绝、基础余额预检拒绝、用户分组访问控制拒绝、无可用候选拒绝和 Redis 全局/IP/Token/User/Model/Channel 限流拒绝分支的基础 policy_snapshot 与 `rate_limit_snapshot`、含过滤/模型重写/重试摘要的基础 route_snapshot、含价格表达式或 P0 回退表达式/规则版本/倍率/预算前后摘要的基础 billing_snapshot 和含 HTTP 请求量/耗时、Relay 请求数、Relay/上游耗时、Relay 错误维度、token 用量、额度消耗维度、API Key 鉴权/生命周期/最近使用/额度/轮换/泄露、通道可用状态、逐通道错误计数、后台探测结果、DB/Redis 错误计数、日志补写 outbox 状态、限流拒绝、计费失败、审计事件计数的基础 `/metrics` 可查；后续补更多指标、更多审计动作和生产 `/ready` 完整可用 | 新增 readiness/metrics 和更多审计动作测试 |
| WP2-3 支付和充值 | P2 | payment products/orders/events、quota_transactions、webhook | 支付商品管理及审计、支付订单创建审计、充值码生成/导入/作废/兑换审计、充值码批次/备注/过期策略、本地 pending 订单、Stripe Checkout Session 创建、Stripe/易支付 provider 退款请求、Stripe webhook 入账和明确失败审计、Stripe 全额/部分退款和扣回审计、Stripe 争议生命周期和可选 API Key 禁用审计、易支付异步通知审计、支付人工补账/扣回、支付人工退款落账和同步返回已覆盖；后续补更多 provider 自动发起退款流程 | 新增更多 provider 自动退款测试 |
...
| 15 | API Key/channel-group scope 合法、忽略、拒绝和无候选路径 | 已覆盖：`TestRouterXRequestFieldIsIgnoredForRoutingAndStripped` | 证明用户偏好不能绕过管理员策略。 |
| 16 | SSE 流式、客户端断开和流式 usage 结算 | 部分覆盖：OpenAI Chat/Completions/Responses 基础 SSE、Anthropic Messages Stream/Gemini streamGenerateContent 到 OpenAI-compatible SSE、`TestAnthropicMessagesStreamToAnthropicUpstreamPreservesNativeSSEAndDeductsUsage`、`TestGeminiStreamGenerateContentToGeminiUpstreamPreservesNativeSSEAndDeductsUsage`、usage 扣费、客户端断开取消和非 OpenAI SSE 通道拒绝已覆盖；仍需更完整 usage fallback/估算策略 | 进入 P1 前补齐最容易出现资源泄漏和账单偏差的路径。 |
| 17 | Anthropic/Gemini 入口错误格式和字段降级 | 部分覆盖：API Key 错误外形、非流式成功、字段降级、Anthropic/Gemini 基础流式、基础下游错误外形、`TestAnthropicMessagesToAnthropicUpstreamPreservesNativeRequestFieldsAndDeductsUsage` 和 `TestGeminiGenerateContentToGeminiUpstreamPreservesNativeFields` 已覆盖；仍需完整 SDK 行为矩阵 | 证明多入口协议不是只注册路由，而是 SDK 可用。 |
