...
- 充值码已支持用户兑换未使用码并写入额度流水，生成/导入/作废已写管理审计；支付商品管理、本地 pending 订单、Stripe webhook、Stripe 全额退款基础处理、易支付异步通知和同步返回基础能力已可用，API Key 生命周期、轮换、泄露上报、用量摘要、管理员查询/批量禁用、用户管理、支付商品管理、settings 更新、用户调额、通道管理、管理员账号管理和日志清理审计已具备基础切片，部分退款、争议和全量支付审计仍需补齐，契约以 `docs/PAYMENTS.md` 为准。
...
| P2-2 | 生产观测、安全和审计增强 | API Key 生命周期/轮换/泄露/批量禁用审计、用户管理、支付商品管理、settings 更新、用户调额、充值码管理、通道管理、管理员账号管理、日志清理审计和基础 `/metrics` 切片已完成，结构化日志、更多指标、告警和完整审计覆盖待完善 |
...
- 管理审计日志，当前已覆盖 API Key 生命周期、轮换、泄露上报、批量禁用、用户管理、支付商品管理、settings 更新、用户调额、充值码管理、通道管理、管理员账号管理和日志清理基础操作，后续覆盖价格、日志导出、支付事件等关键操作。
...
| WP2-2 生产观测和审计 | P2 | structured logs、metrics、audit、ready | API Key 生命周期、轮换、泄露上报、批量禁用、用户管理、支付商品管理、settings 更新、用户调额、充值码管理、通道管理、管理员账号管理、日志清理审计和基础 `/metrics` 可查；后续补 Request ID、更多指标、更多审计动作和生产 `/ready` 完整可用 | 新增 readiness/metrics 和更多审计动作测试 |
