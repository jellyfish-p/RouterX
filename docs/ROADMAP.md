...
RouterX 当前已经不再只是后端骨架。代码已具备可验证的 P0 开箱闭环，并扩展到多协议入口、主要上游适配、基础流式、访问控制、可靠性、独立日志库、观测审计、支付/充值码和高级 API 的基础链路；剩余工作按追踪矩阵作为后续增强继续管理，不作为当前不扩功能收口的阻塞。
...
- 用户、管理员、用户分组、API Key、通道、日志、设置、看板、审计、模型价格、支付和充值码等接口。
...
- 充值码已支持用户兑换未使用码并写入额度流水，生成/导入/作废已写管理审计；支付商品管理、本地 pending 订单、Stripe Checkout Session 创建、Stripe webhook 入账和明确失败审计、Stripe 争议生命周期记录和可选 API Key 禁用、易支付异步通知审计、支付人工补账/扣回和同步返回基础能力已可用，API Key 生命周期、轮换、泄露上报、用量摘要、元数据和服务账号主体过滤/脱敏导出、管理员查询/批量禁用/批量过期、用户管理、支付商品管理、settings 更新和校验拒绝、用户调额、通道管理、管理员账号管理和日志清理审计已具备基础切片，契约以 `docs/PAYMENTS.md` 为准。
...
| P1-3 | 计费策略、访问控制、充值码和可选支付插件 | 基础闭环已覆盖；长尾历史解释继续跟踪 |
