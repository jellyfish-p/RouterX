...
- 充值码已支持用户兑换未使用码并写入额度流水；支付商品管理、本地 pending 订单、Stripe webhook、易支付异步通知和同步返回基础能力已可用，退款和全量支付审计仍需补齐，契约以 `docs/PAYMENTS.md` 为准。
...
| WP2-3 支付和充值 | P2 | payment products/orders/events、quota_transactions、webhook | 支付商品管理、本地 pending 订单、Stripe webhook、易支付异步通知和同步返回已覆盖；后续补退款和人工修正流水审计 | 新增退款和人工修正测试 |
