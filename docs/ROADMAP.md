...
- 结构化日志、更多 Prometheus 指标、更严格 `/ready`；调用日志 request_id/error_code/usage_source/error_source/upstream_status/基础 route_snapshot/基础 billing_snapshot 已有基础持久化。
...
| WP1-4 计费规则和访问控制 | P1 | model_prices、channel_model_prices、settings、logs、POLICIES | 价格表达式、倍率、访问控制、计费快照和账单聚合一致 | 基础 `billing_snapshot` 已记录结算状态、usage_source、token 事实和最终扣费；价格表达式、倍率和访问控制快照仍需补齐 |
