...
- 可配置 HTTP/Panic 结构化日志、Prometheus 指标、完整管理审计覆盖和告警。
...
| P1-3 | 计费策略、访问控制、充值码和可选支付插件 | 充值码生成/导入/作废/兑换、批次/备注/过期策略、管理员调额、支付商品管理审计、支付入账、provider 明确失败、全额和部分退款回调审计、Stripe 争议生命周期审计、Stripe/易支付 provider 退款请求审计、支付人工修正和人工退款审计、settings 更新和校验拒绝审计、用户调额审计和充值码管理审计已写基础闭环；价格策略、访问控制和更多 provider 自动发起退款流程待完善，策略契约见 `docs/POLICIES.md`，支付契约见 `docs/PAYMENTS.md` |
| P2-1 | OAuth/OIDC 和企业账号体系 | 待规划实现 |
| P2-2 | 生产观测、安全和审计增强 | API Key 生命周期/轮换/泄露/元数据和服务账号主体过滤/脱敏导出/批量禁用/批量过期审计、用户管理、支付商品管理、支付入账/明确失败/退款回调、Stripe/易支付 provider 退款请求、Stripe 争议生命周期、支付人工修正、settings 更新和校验拒绝、用户调额、充值码管理、通道管理、管理员账号管理、日志清理/导出审计、可配置 HTTP/Panic 结构化日志和含 HTTP 请求量/耗时、Relay 请求数、Relay/上游耗时、Relay 错误维度、token 用量、额度消耗维度、API Key 鉴权/生命周期/最近使用/额度/轮换/泄露、通道可用状态、逐通道错误计数、日志补写 outbox 状态、DB/Redis 错误计数、限流拒绝、计费失败、审计事件计数的基础 `/metrics` 切片已完成，更多结构化字段、更多指标、告警和完整审计覆盖待完善 |
