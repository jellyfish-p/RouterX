...
- 充值码已支持用户兑换未使用码并写入额度流水；支付商品和本地 pending 订单基础接口已可用，在线支付插件、支付回调幂等、全量支付审计仍需补齐，契约以 `docs/PAYMENTS.md` 为准。
...
| WP2-3 支付和充值 | P2 | payment products/orders/events、quota_transactions、webhook | 支付商品和本地 pending 订单已覆盖；后续支付回调签名、金额、状态和幂等入账正确；重复通知不重复加额度；退款和人工修正有流水和审计 | 新增支付幂等、额度流水和人工修正测试 |
