...
- 充值码已支持用户兑换未使用码并写入额度流水；在线支付插件、支付回调幂等、全量支付审计仍需补齐，契约以 `docs/PAYMENTS.md` 为准。
...
| P1-3 | 计费策略、访问控制、充值码和可选支付插件 | 充值码用户兑换和管理员调额已写额度流水；价格策略、访问控制、充值码管理端、支付插件和审计待完善，策略契约见 `docs/POLICIES.md`，支付契约见 `docs/PAYMENTS.md` |
...
- 充值码用户兑换基础接口和额度流水；后续补充值码管理端和可选支付插件接口，按 `docs/PAYMENTS.md` 保证订单、事件和审计。
