...
- 客户端断开、Anthropic/Gemini 流式转换、重试、熔断、限流的生产级闭环。
...
| P1-1 | 流式响应、重试、熔断和限流 | 部分推进，OpenAI Chat 基础 SSE 已覆盖 |
...
| 2 | Chat 非流式本地请求错误和下游 400/401/403/5xx/超时错误映射 | 已覆盖：`TestChatCompletionInvalidRequestDoesNotCallUpstream`、`TestChatCompletionUpstreamBadRequestMapping`、`TestChatCompletionUpstreamErrorStatusMapping`、`TestChatCompletionUpstreamTimeoutMapping` | 证明 SDK 兼容错误、重试边界和通道故障归因正确。 |
