...
- 充值码已支持用户兑换未使用码并增加余额；在线支付插件、支付回调幂等、统一额度流水和支付审计仍需补齐，契约以 `docs/PAYMENTS.md` 为准。
...
| P1-3 | 计费策略、访问控制、充值码和可选支付插件 | 充值码用户兑换基础闭环已完成；价格策略、访问控制、充值码管理端、额度流水和支付插件待完善，策略契约见 `docs/POLICIES.md`，支付契约见 `docs/PAYMENTS.md` |
...
- 充值码用户兑换基础接口；后续补充值码管理端和可选支付插件接口，按 `docs/PAYMENTS.md` 保证订单、事件、额度流水和审计。
