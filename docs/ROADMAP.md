...
- 结构化日志、更多 Prometheus 指标、更严格 `/ready`；调用日志 request_id/error_code/usage_source/error_source/upstream_status/含模型重写摘要的基础 route_snapshot/含预算前后摘要的基础 billing_snapshot 已有基础持久化。
...
| WP1-2 路由决策快照 | P1 | channel selection、route policy、logs、POLICIES | 记录候选过滤、`routerx.route` 处理、最终通道、模型重写和重试结果 | 基础 `route_snapshot` 已记录请求模型、候选数量、最终通道、provider、分组、优先级、权重和模型重写摘要；`routerx.route` 边界已由 `TestRouterXRoutePreferenceFiltersChannels` 覆盖；过滤原因和重试摘要仍需补齐 |
