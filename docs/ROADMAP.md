...
| WP2-3 支付和充值 | P2 | payment products/orders/events、quota_transactions、webhook | 支付商品管理及审计、支付订单创建审计、充值码生成/导入/作废/兑换审计、本地 pending 订单、Stripe webhook 入账审计、Stripe 全额/部分退款和扣回审计、Stripe 争议事件记录和可选 API Key 禁用审计、易支付异步通知审计、支付人工补账/扣回和同步返回已覆盖；后续补完整争议生命周期和更完整人工退款流程 | 新增完整争议生命周期和人工退款流程测试 |
