...
- 充值码已支持用户兑换未使用码并写入额度流水，生成/导入/作废已写管理审计；支付商品管理、本地 pending 订单、Stripe Checkout Session 创建、Stripe webhook 入账审计、Stripe 全额/部分退款和扣回审计、Stripe 争议事件记录和可选 API Key 禁用、易支付异步通知审计、支付人工补账/扣回和同步返回基础能力已可用，API Key 生命周期、轮换、泄露上报、用量摘要、管理员查询/批量禁用/批量过期、用户管理、支付商品管理、settings 更新、用户调额、通道管理、管理员账号管理和日志清理审计已具备基础切片，完整争议生命周期和更完整人工退款流程仍需补齐，契约以 `docs/PAYMENTS.md` 为准。
...
| P1-3 | 计费策略、访问控制、充值码和可选支付插件 | 充值码生成/导入/作废/兑换、管理员调额、支付商品管理审计、支付入账/全额和部分退款回调审计、支付争议事件审计、支付人工修正审计、settings 更新审计、用户调额审计和充值码管理审计已写基础闭环；价格策略、访问控制、完整争议生命周期和更完整人工退款流程待完善，策略契约见 `docs/POLICIES.md`，支付契约见 `docs/PAYMENTS.md` |
| P2-1 | OAuth/OIDC 和企业账号体系 | 待规划实现 |
| P2-2 | 生产观测、安全和审计增强 | API Key 生命周期/轮换/泄露/批量禁用/批量过期审计、用户管理、支付商品管理、支付入账/退款回调、支付人工修正、settings 更新、用户调额、充值码管理、通道管理、管理员账号管理、日志清理审计和含 HTTP 请求量/耗时、Relay 请求数、Relay/上游耗时、Relay 错误维度、token 用量、额度消耗维度、通道可用状态、逐通道错误计数、限流拒绝、计费失败、审计事件计数的基础 `/metrics` 切片已完成，结构化日志、更多指标、告警和完整审计覆盖待完善 |
