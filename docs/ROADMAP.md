...
- 系统模型价格表管理已落地；仍需补通道价格覆盖、表达式热路径执行、分组倍率、访问控制和计费快照，策略契约见 `docs/POLICIES.md`。
...
- `model_prices` 已支持系统价格管理、版本和用户侧价格状态；仍需补 `channel_model_prices`、通道分组、用户分组倍率、访问控制和表达式执行。
...
- 管理审计日志，当前已覆盖 API Key 生命周期、轮换、泄露上报、批量禁用、批量过期、用户管理、支付商品管理、系统模型价格管理、settings 更新、用户调额、充值码管理、通道管理、管理员账号管理和日志清理基础操作，后续覆盖通道价格、日志导出、支付事件等关键操作。
...
| WP1-4 计费规则和访问控制 | P1 | model_prices、channel_model_prices、settings、logs、POLICIES | 价格表达式、倍率、访问控制、计费快照和账单聚合一致 | 基础 `billing_snapshot` 已记录结算状态、usage_source、P0 表达式摘要、默认倍率摘要、Key 预算前后、用户余额前后和最终扣费；系统模型价格表管理、规则版本、用户侧价格就绪状态和审计已完成；通道价格覆盖、表达式热路径执行、业务倍率和访问控制快照仍需补齐 |
