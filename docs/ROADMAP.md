...
- 充值码已支持用户兑换未使用码并写入额度流水；支付商品管理、本地 pending 订单、Stripe webhook、Stripe 全额退款基础处理、易支付异步通知和同步返回基础能力已可用，部分退款、争议和全量支付审计仍需补齐，契约以 `docs/PAYMENTS.md` 为准。
...
| P1-3 | 计费策略、访问控制、充值码和可选支付插件 | 充值码生成/导入/作废、用户兑换和管理员调额已写基础闭环；价格策略、访问控制、支付插件和审计待完善，策略契约见 `docs/POLICIES.md`，支付契约见 `docs/PAYMENTS.md` |
...
| WP2-3 支付和充值 | P2 | payment products/orders/events、quota_transactions、webhook | 支付商品管理、本地 pending 订单、Stripe webhook、Stripe 全额退款基础处理、易支付异步通知和同步返回已覆盖；后续补部分退款、争议和人工修正流水审计 | 新增部分退款、争议和人工修正测试 |
