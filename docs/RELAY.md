...
- Image Generations 已按 Azure `/openai/v1/images/generations?api-version=preview` 调用；请求体保留 `model` 作为 deployment 名，剥离 RouterX 私有字段，并在上游前校验 `size` 为缺省、空字符串、`auto` 或合法 `WIDTHxHEIGHT`。
