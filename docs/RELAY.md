...
- OpenAI-compatible multipart 文件名命中路径形态、危险扩展名、图片/音频 API 不匹配的文件扩展名，或内容命中可执行/脚本签名基础扫描时，本地返回 400 `unsafe_multipart_file`，且不选择或调用上游、不扣费。
- 需要记录下游状态码。
...
- 请求体日志默认关闭。
- 图片、音频 multipart 文件字段受 `relay.max_multipart_file_bytes` 限制，并会拒绝路径形态文件名、明显可执行或脚本类危险扩展名、图片/音频 API 不匹配的文件扩展名，以及明显可执行或脚本类内容签名。
- 对下游响应体设置最大读取大小。
