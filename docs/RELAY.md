...
- Anthropic/Gemini 非流式入口已覆盖基础成功响应、usage、扣费和非文本 content/parts 的可解释降级；Gemini generateContent 命中 Gemini 上游时已覆盖非流式原生字段保真；降级摘要会写入脱敏的 `request_snapshot.adapter_degradations`。
...
| Gemini | Gemini | generateContent 非流式原生 body 保留；原生上游流式继续补齐 |
...
- Gemini 原生入口请求命中 Gemini 上游时，会保持 `contents`、`systemInstruction`、`generationConfig`、`safetySettings`、`tools`、`toolConfig` 和 `cachedContent`；命中 OpenAI-compatible 上游时仍按 Chat 形态转换并记录不可映射字段降级。
