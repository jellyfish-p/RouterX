...
- 当前已支持进程内候选缓存、Redis 共享候选快照、启动预热和短 TTL，由 `routing.channel_cache.enabled`、`routing.channel_cache.preload`、`routing.channel_cache.ttl_seconds` 和 `routing.channel_cache.version` 控制。
...
- 管理员修改模型、通道分组、用户分组访问控制、价格规则或相关 settings 后，基于 `routing.channel_cache.version` 的跨实例被动失效已可用；主动 pub/sub 广播仍属于集群增强。
...
- 当前通道候选缓存会缓存排序后的通道表快照，并在服务启动和通道版本变更后按 `routing.channel_cache.preload` 预热；Redis 可用时还会共享同一份候选快照。模型匹配、熔断、`routerx.route`、用户分组访问和 API Key scope 仍在请求路径实时过滤。
