...
- 非流式安全重试已经按 `relay.retry_count` 和 `relay.retry_on_status` 支持基础换候选；Redis 限流已覆盖全局、IP、Token、用户和模型的分钟窗口；`error_count` 熔断和冷却半开候选探测已落地；后台探测任务、通道限流维度、计费快照和可观测指标仍需要生产级实现。
...
- 当前使用 Redis 固定分钟窗口计数器，读取 `rate_limit.enabled`、`rate_limit.global_per_min`、`rate_limit.per_ip_per_min`、`rate_limit.per_token_per_min`、`rate_limit.per_user_per_min` 和 `rate_limit.per_model_per_min`。
