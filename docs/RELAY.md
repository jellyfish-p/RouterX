...
- OpenAI-compatible SSE 流式已覆盖客户端写入失败后的上游取消断言；Anthropic/Gemini chunk 转换和更完整的 usage fallback 策略仍需补齐。
...
- Image Edits/Variations、Audio 等高级接口已注册或纳入设计，但仍需逐接口补齐 multipart、二进制响应和大小限制；Responses 仍需继续补完整原生字段矩阵和多上游转换，Embeddings 仍需补输入 schema、批量边界和模型能力策略，Image Generations/Moderations 仍需补完整安全策略和价格规则。
