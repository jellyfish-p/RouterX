...
- `routerx.upstream` 已支持基础安全 header/query 补充和 JSON body 缺省字段补充；Adapter 接口仍需要扩展为显式结构化选项，以覆盖完整 provider-specific 命名空间、流式 chunk 和更细粒度错误。
...
- xAI 特有 JSON 参数当前可通过 `routerx.upstream.body` 补充到原请求缺失字段；目标设计中的 `routerx.provider.xai` 命名空间仍按 provider-specific 矩阵继续补齐。
