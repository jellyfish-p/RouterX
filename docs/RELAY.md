...
- 请求级 upstream/provider 扩展已移除；请求体或 multipart 表单中的 `routerx` 字段、以及 `X-RouterX-Options` header 不参与路由或上游参数补充。Anthropic Messages 原生入口命中 Anthropic 上游时，会保留 `system/messages/max_tokens/metadata/stop_sequences/stream/temperature/top_p/top_k/tools/tool_choice/thinking` 等白名单字段；Gemini generateContent 原生入口命中 Gemini 上游时，会保留 `contents/systemInstruction/generationConfig/safetySettings/tools/toolConfig/cachedContent`，成功日志保留真实入口协议并避免把这些字段误记为 dropped。Adapter 接口仍需要扩展为显式结构化选项，以覆盖完整 provider 字段映射、流式 chunk 和更细粒度错误。
...
- Responses 已支持 OpenAI-compatible JSON/SSE 透传、Azure `/openai/v1/responses?api-version=preview` JSON 转发、Claude/Anthropic 通道的基础非流式 Responses 转 Messages，以及 Gemini 通道的基础非流式 Responses 转 `generateContent`；Responses SSE 会从 `response.completed.response.usage` 提取用量并扣费，Claude Responses 会从 Messages `usage.input_tokens/output_tokens` 写回 Responses usage，Gemini Responses 会从 `usageMetadata` 写回 Responses usage。仍需继续补完整原生事件矩阵和更多上游转换；Image Edits/Variations 已支持 OpenAI-compatible multipart 表单基础透传、必填 `image` 文件字段校验、`size` 基础尺寸边界校验、`routerx` 表单字段剥离且不参与路由、`relay.max_multipart_file_bytes` 单文件字段限制、路径形态、危险扩展名、非图片扩展名、非图片文件头和可执行/脚本内容签名基础扫描、最低计费，其中 Azure Image Edits/Variations 已支持 `/openai/v1/images/edits|variations?api-version=preview`。仍需补完整内容安全扫描、字段保真和价格规则；Audio Transcriptions/Translations 已支持 OpenAI-compatible multipart 表单基础透传、必填 `file` 文件字段校验、`response_format` 基础枚举校验、`routerx` 表单字段剥离且不参与路由、`relay.max_multipart_file_bytes` 单文件字段限制、路径形态、危险扩展名、非音频扩展名、非音频文件头和可执行/脚本内容签名基础扫描、最低计费，仍需补更完整时长限制和内容安全扫描；Audio Speech 已支持 JSON 请求、`input/voice` 基础契约校验、`response_format` 基础枚举校验和二进制响应基础透传，仍需补完整价格规则；Embeddings 已支持上游前 input schema 和 2048 批量边界校验，仍需补更细模型能力策略；Image Generations 已支持上游前 `prompt` 必填文本、`n` 正整数和 `size` 基础尺寸边界校验，非法分别返回 `invalid_image_prompt`、`invalid_image_count` 或 `invalid_image_size` 且不上游、不扣费；Moderations 已支持上游前 `input` 基础契约校验，非法返回 `invalid_moderation_input` 且不上游、不扣费，仍需补完整安全策略和价格规则。
...
- 客户端请求体、multipart 表单或 header 中的 `routerx` 扩展参数不再参与路由选择或上游参数补充。
- Adapter 调用真实厂商前必须移除 `routerx` 字段；RouterX-Compatible 上游也只转发 hop/chain 等内部 header，不依赖请求体 `routerx` 字段。
- 禁止透传敏感 header，如 `Authorization`、`Cookie`、`Set-Cookie`、`X-Api-Key`、`api-key`、`Content-Type` 和 `X-RouterX-*`，这些必须来自通道配置或 RouterX 自身生成。
- multipart 请求会在转发前剥离 `routerx` 表单字段；`X-RouterX-Options` header 会被忽略。
- Anthropic/Gemini 原生入口命中同 provider 上游时，通过内部上下文保留协议原生白名单字段，不通过 请求级 provider 扩展 传递。
...
- provider、通道分组或 API Key scope 越权时不自动降级到无权通道。
- 请求携带 `routerx` 字段或 `X-RouterX-Options` header 不再触发专门的 400 解析错误。
- 无可用候选通道返回入口协议兼容的无可用通道错误；如果原因是访问策略禁止，应返回 403。
- 路由快照记录候选过滤、最终通道和模型重写，不记录请求级 routerx 偏好。
...
- 上游类型为 `routerx` 时，当前实现会递增并转发 `X-RouterX-Hop`，并追加 `X-RouterX-Chain`；请求体中的 `routerx` 字段仍会被剥离。
...
|------|-----------|------|----------|----------|--------|
| 请求格式 | `invalid_json`、`invalid_chat_messages`、`model_required`、`unsupported_api` | 400/404 | 否 | 否 | 调用方 |
| 鉴权状态 | `invalid_api_key`、`expired_api_key`、`user_disabled` | 401/403 | 否 | 否 | 用户或管理员 |
| 额度和限流 | `insufficient_quota`、`rate_limit_exceeded`、`rate_limit_unavailable` | 429/503 | 否 | 否 | 用户或管理员 |
...
|-----------|------------|------------|--------------|
| `invalid_*`、`model_required` | 修正请求体或模型名 | 不需要介入，除非大量出现 | request_id、入口协议、解析失败字段 |
| `invalid_api_key`、`expired_api_key` | 更换或重新创建 API Key | 检查用户和 API Key 状态 | user_id、token_id、失败原因摘要 |
| `insufficient_quota`、`rate_limit_exceeded`、`rate_limit_unavailable` | 充值、降低并发、等待窗口或等待 Redis 恢复 | 调整用户额度、Key 预算、限流配置或 Redis 连接 | user quota、key budget、限流 key、Redis 依赖状态 |
| `route_forbidden` | 联系管理员调整权限或通道分组 | 检查通道分组、用户分组和访问策略 | API Key/channel-group scope 摘要、拒绝原因 |
...
路由决策必须先做不可绕过的系统过滤和访问控制，最后才进入优先级和权重选择。推荐顺序如下：
完整策略语义见 `docs/POLICIES.md`。

```text
1. 解析入口协议、API 类型、请求模型和 stream 标记
...
- API Key scope、用户分组和通道分组访问控制在缓存候选集之后应用，避免生成高基数缓存。
- 缓存不可绕过数据库最终事实；版本不一致时不得长期使用旧候选集。

当前代码事实：

- 当前通道查询已经按 `priority DESC, idx ASC, error_count ASC, response_ms ASC, id ASC` 排序。
- 当前通道候选缓存会缓存排序后的通道表快照，并在服务启动和通道版本变更后按 `routing.channel_cache.preload` 预热；Redis 可用时还会共享同一份候选快照，并用 pub/sub 主动通知其他实例清理本地快照。模型匹配、熔断、用户分组访问和 API Key scope 仍在请求路径实时过滤。
...
- 单 `api_key` 与 `api_keys` 同时存在时，服务层把两类配置合并为候选 key 集，但日志和管理端应提示配置来源。
...
| OpenAI | Gemini | Chat 和基础非流式 Responses 转 contents/generationConfig/systemInstruction；Responses 的工具、视觉和推理项仍需补齐 |
...
- Image Generations 已按 Azure `/openai/v1/images/generations?api-version=preview` 调用；请求体保留 `model` 作为 deployment 名，剥离 RouterX 私有字段，并在上游前校验 `prompt` 为非空字符串、显式 `n` 为正整数、`size` 为缺省、空字符串、`auto` 或合法 `WIDTHxHEIGHT`。
...
- Audio Speech 已按 Azure `/openai/v1/audio/speech?api-version=preview` 调用；请求体保留 `model` 作为 deployment 名，剥离 RouterX 私有字段，上游前校验 `input`、`voice` 和 `response_format` 基础契约，并透传上游音频 Content-Type。
...
- OpenAI-compatible Chat 标准生成参数会映射到 Gemini 原生 `generationConfig`；请求体中的 routerx provider 扩展不会注入 Gemini `safetySettings`。
...
- xAI 特有 JSON 参数需要通过入口协议本身支持的字段或后续显式 adapter 结构化选项传递；请求体中的 routerx provider/upstream 扩展不会补充到上游 body。
...
- 上游也是 RouterX 或兼容 RouterX hop/chain header 的网关。
- 通过 `X-RouterX-Hop` 和 `X-RouterX-Chain` 做循环保护与链路摘要；请求体 `routerx` 字段仍会被剥离。
...
- OpenAI-compatible Images/Audio multipart 缺少必填 `image` 或 `file` 文件字段时，本地返回 400 `multipart_file_required`，且不选择或调用上游、不扣费。
- OpenAI-compatible multipart 单个文件字段超过 `relay.max_multipart_file_bytes` 时，本地返回 413 `request_file_too_large`，且不选择或调用上游、不扣费。
- OpenAI-compatible multipart 文件名命中路径形态、危险扩展名、图片/音频 API 不匹配的文件扩展名或文件头，或内容命中可执行/脚本签名基础扫描时，本地返回 400 `unsafe_multipart_file`，且不选择或调用上游、不扣费。
- 需要记录下游状态码。
...
- P1 可升级令牌桶或漏桶。
- Redis 不可用时 SQLite 单镜像可降级；外部数据库或集群模式下全局/IP/Token/User/Model/Channel 限流会在上游调用前返回 `rate_limit_unavailable` 并 fail-closed。
...
- 请求体日志默认关闭。
- 图片、音频 multipart 文件字段受 `relay.max_multipart_file_bytes` 限制，并会拒绝路径形态文件名、明显可执行或脚本类危险扩展名、图片/音频 API 不匹配的文件扩展名或文件头，以及明显可执行或脚本类内容签名。
- 对下游响应体设置最大读取大小。
...
| 通道选择 | 模型匹配、启用状态、优先级、权重和无可用通道可测 | API Key/channel-group scope、访问控制、熔断排除、路由快照可测 |
