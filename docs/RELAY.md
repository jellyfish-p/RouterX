...
- Image Edits/Variations、Audio Transcriptions/Translations 等高级接口已注册或纳入设计，但仍需逐接口补齐 multipart、文件上传和大小限制；Audio Speech 已支持 JSON 请求和二进制响应基础透传，但仍需补完整音频格式策略、时长限制和价格规则；Responses 仍需继续补完整原生字段矩阵和多上游转换，Embeddings 仍需补输入 schema、批量边界和模型能力策略，Image Generations/Moderations 仍需补完整安全策略和价格规则。
