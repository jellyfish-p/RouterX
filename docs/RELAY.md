...
- Audio Speech 已按 Azure `/openai/v1/audio/speech?api-version=preview` 调用；请求体保留 `model` 作为 deployment 名，剥离 RouterX 私有字段，上游前校验 `response_format` 为缺省、空字符串或支持的音频容器格式，并透传上游音频 Content-Type。
