...
- OpenAI-compatible SSE 流式已覆盖客户端写入失败后的上游取消断言；Gemini streamGenerateContent 已支持转 OpenAI-compatible Chat SSE 上游并输出 Gemini SSE 事件；Anthropic 流式、Gemini 原生上游流式和更完整的 usage fallback 策略仍需补齐。
