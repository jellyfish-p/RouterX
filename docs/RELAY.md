...
- 非流式安全重试已经按 `relay.retry_count` 和 `relay.retry_on_status` 支持基础换候选；Redis 限流已覆盖全局、IP 和 Token 的分钟窗口；熔断、更多限流维度、计费快照和可观测指标仍需要生产级实现。
...
| 重试熔断 | 非流式命中 `relay.retry_on_status`、网络错误或超时可按 `relay.retry_count` 换候选；默认 400/401/403 不重试 | 半开恢复、限流、更多故障注入和流式不可切换通道可测 |
