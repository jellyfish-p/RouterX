...
- 当前已支持进程内候选缓存和短 TTL，由 `routing.channel_cache.enabled`、`routing.channel_cache.ttl_seconds` 和 `routing.channel_cache.version` 控制。
- 管理员通过通道管理接口创建、编辑、启用、禁用或删除通道后，会递增 `routing.channel_cache.version` 并让当前进程缓存失效。
...
- 当前通道候选缓存会缓存排序后的通道表快照；模型匹配、熔断、`routerx.route`、用户分组访问和 API Key scope 仍在请求路径实时过滤。
