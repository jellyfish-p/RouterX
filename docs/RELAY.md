...
- Responses 已支持 OpenAI-compatible JSON/SSE 透传和 Azure `/openai/v1/responses?api-version=preview` JSON 转发，Responses SSE 会从 `response.completed.response.usage` 提取用量并扣费，仍需继续补完整原生事件矩阵和更多上游转换；Image Edits/Variations 已支持 OpenAI-compatible multipart 表单基础透传、`routerx` 表单字段或 `X-RouterX-Options` header 路由偏好和最低计费，其中 Azure Image Edits 已支持 `/openai/v1/images/edits?api-version=preview`；Azure Image Variations 端点尚未验证。仍需补文件大小限制、安全扫描、字段保真和价格规则；Audio Transcriptions/Translations 已支持 OpenAI-compatible multipart 表单基础透传、`routerx` 表单字段或 `X-RouterX-Options` header 路由偏好和最低计费，仍需补完整音频格式策略、时长限制、文件大小限制和价格规则；Audio Speech 已支持 JSON 请求和二进制响应基础透传，但仍需补完整音频格式策略、时长限制和价格规则；Embeddings 已支持上游前 input schema 和 2048 批量边界校验，仍需补更细模型能力策略；Image Generations/Moderations 仍需补完整安全策略和价格规则。
