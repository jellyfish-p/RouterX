...
3. 应用 API Key scope；当前 `allow_models` 未命中会返回 `model_not_allowed`，`api_types`、`ip_cidrs` 或 `methods` 未命中会返回 `token_forbidden`，`channel_groups` 未命中会返回 `route_forbidden`，`daily_quota` 到达上限会返回 `insufficient_quota`，且都不调用上游
...
- 当前 API Key scope 支持 `allow_models` 模型 allow-list、`api_types` APIType allow-list、`channel_groups` 通道分组 allow-list、`ip_cidrs` IP/CIDR allow-list、`methods` 方法路径 allow-list 和 `daily_quota` 日预算，并在通道候选和上游调用前拒绝未允许的模型、接口能力、通道分组、来源 IP、方法路径或已达日预算请求。
