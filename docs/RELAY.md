...
3. 应用 API Key scope；当前 `allow_models` 未命中会返回 `model_not_allowed`，`api_types` 或 `ip_cidrs` 未命中会返回 `token_forbidden`，`channel_groups` 未命中会返回 `route_forbidden`，且都不调用上游
...
- 当前 API Key scope 支持 `allow_models` 模型 allow-list、`api_types` APIType allow-list、`channel_groups` 通道分组 allow-list 和 `ip_cidrs` IP/CIDR allow-list，并在通道候选和上游调用前拒绝未允许的模型、接口能力、通道分组或来源 IP。
