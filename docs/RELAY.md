...
- 非流式安全重试已经按 `relay.retry_count` 和 `relay.retry_on_status` 支持基础换候选；Redis 限流已覆盖全局、IP、Token、用户、模型和通道的分钟窗口，拒绝日志会写入基础 `policy_snapshot` 和 `rate_limit_snapshot`；`error_count` 熔断和冷却半开候选探测已落地；后台探测任务、完整熔断快照、计费快照和可观测指标仍需要生产级实现。
