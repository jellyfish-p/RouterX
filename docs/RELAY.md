...
3. 应用 API Key scope；当前 `allow_models` 未命中会返回 `model_not_allowed`，`api_types`、`entry_protocols`、`ip_cidrs` 或 `methods` 未命中会返回 `token_forbidden`，`channel_groups` 未命中会返回 `route_forbidden`，`daily_quota` 或 `monthly_quota` 到达上限会返回 `insufficient_quota`，`max_concurrency` 达到上限会返回 `rate_limit_exceeded`，且都不调用上游
