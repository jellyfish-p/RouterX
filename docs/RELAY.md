...
- `RelayService` 已有通道选择、上游解析、非流式转发、OpenAI-compatible Chat/Legacy Completions 基础 SSE 转发、Responses 基础 JSON 透传、模型重写、usage 提取、扣费和日志基础链路。
...
| 响应转换 | 成功响应保留入口协议格式，usage 能提取；OpenAI Chat/Completions 基础 SSE、Responses 基础 usage 映射和 Anthropic/Gemini 基础非流式已覆盖 | 流式 chunk、tool calling、vision、reasoning 和降级原因可测 |
