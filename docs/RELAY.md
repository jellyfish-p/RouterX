...
- OpenAI-compatible multipart 文件名命中危险扩展名基础扫描时，本地返回 400 `unsafe_multipart_file`，且不选择或调用上游、不扣费。
- 需要记录下游状态码。
...
- 请求体日志默认关闭。
- 图片、音频 multipart 文件字段受 `relay.max_multipart_file_bytes` 限制，并会拒绝明显可执行或脚本类危险扩展名。
- 对下游响应体设置最大读取大小。
