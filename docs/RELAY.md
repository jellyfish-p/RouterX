...
- `routerx.upstream` 已支持基础安全 header/query 补充和 JSON body 缺省字段补充，`routerx.provider.<provider>` 已支持按选中上游合并 provider 专属 JSON body 缺省字段；Adapter 接口仍需要扩展为显式结构化选项，以覆盖完整 provider 字段映射、流式 chunk 和更细粒度错误。
- 非流式安全重试已经按 `relay.retry_count` 和 `relay.retry_on_status` 支持基础换候选；Redis 限流已覆盖全局、IP、Token 和用户的分钟窗口；`error_count` 熔断和冷却半开候选探测已落地；后台探测任务、模型/通道限流维度、计费快照和可观测指标仍需要生产级实现。
- Responses 已支持 OpenAI-compatible JSON 透传和 Azure `/openai/v1/responses?api-version=preview` 转发，仍需继续补完整原生字段矩阵、流式 Responses 和更多上游转换；Image Edits/Variations 已支持 OpenAI-compatible multipart 表单基础透传、`routerx` 表单字段或 `X-RouterX-Options` header 路由偏好和最低计费，其中 Azure Image Edits 已支持 `/openai/v1/images/edits?api-version=preview`；Azure Image Variations 端点尚未验证。仍需补文件大小限制、安全扫描、字段保真和价格规则；Audio Transcriptions/Translations 已支持 OpenAI-compatible multipart 表单基础透传、`routerx` 表单字段或 `X-RouterX-Options` header 路由偏好和最低计费，仍需补完整音频格式策略、时长限制、文件大小限制和价格规则；Audio Speech 已支持 JSON 请求和二进制响应基础透传，但仍需补完整音频格式策略、时长限制和价格规则；Embeddings 仍需补输入 schema、批量边界和模型能力策略，Image Generations/Moderations 仍需补完整安全策略和价格规则。
...
- `routerx.provider.<provider>` 用于传递只对某个上游 provider 生效的额外 JSON 参数；当前实现会在选中 provider 匹配时把这些字段作为 body 缺省补充，且比通用 `routerx.upstream.body` 更具体，但仍不能覆盖原请求字段或内部字段。
...
路径冲突处理：`/v1/models` 同时服务 OpenAI、Anthropic、Gemini 模型列表。当前实现默认返回 OpenAI `models` 结构；协议外形按 `format`、`routerx_protocol`、`X-RouterX-Protocol`、`anthropic-version`、OpenAI 默认值的顺序解析。`format=gemini|anthropic` 保持最高优先级，`routerx_protocol=openai|anthropic|gemini` 和 `X-RouterX-Protocol` 用于无 body 调用方表达 RouterX 协议偏好。
...
- 当前已支持进程内候选缓存、启动预热和短 TTL，由 `routing.channel_cache.enabled`、`routing.channel_cache.preload`、`routing.channel_cache.ttl_seconds` 和 `routing.channel_cache.version` 控制。
- 管理员通过通道管理接口创建、编辑、启用、禁用或删除通道后，会递增 `routing.channel_cache.version`；`routing.channel_cache.preload=true` 时当前进程会立即预热新候选快照，预热失败不阻断管理操作。
- 上游失败导致通道 `error_count` 变化时，当前进程会立即失效候选缓存，保证自动熔断能在下一次选择中生效。
- DB+Redis 或集群模式后续可使用 Redis 保存 `routing.channel_cache.version`、失效标记或共享候选快照。
- 管理员修改模型、通道分组、用户分组访问控制、价格规则或相关 settings 后，跨实例广播失效仍属于集群增强。
...
- 当前通道候选缓存会缓存排序后的通道表快照，并在服务启动和通道版本变更后按 `routing.channel_cache.preload` 预热；模型匹配、熔断、`routerx.route`、用户分组访问和 API Key scope 仍在请求路径实时过滤。
...
- Image Edits 已按 Azure `/openai/v1/images/edits?api-version=preview` 调用；保留 multipart Content-Type、图像/遮罩文件字段和 `model` deployment 名，并剥离 RouterX 私有表单字段。
...
- Azure Image Variations、Moderations 等 API 仍需逐项补齐或等待上游端点确认。
...
- 手动恢复：管理员启用通道。
- 定时探测：后台任务定期测试熔断通道。
- 半开状态：当前已支持冷却窗口后的候选探测，成功调用会清零 `error_count`，失败会继续递增并刷新 `updated_at`。

当前 `channels.status` 只有 `disabled/enabled/manual_off`，自动熔断通过 `error_count` 排除通道实现，不会自动改写 status；后续再增加显式状态字段和后台定时探测任务。
...
- 当前使用 Redis 固定分钟窗口计数器，读取 `rate_limit.enabled`、`rate_limit.global_per_min`、`rate_limit.per_ip_per_min`、`rate_limit.per_token_per_min` 和 `rate_limit.per_user_per_min`。
- 全局、IP、Token、用户四个维度已接入；任一维度阈值为 `0` 时跳过该维度。
...
| 重试熔断 | 非流式命中 `relay.retry_on_status`、网络错误或超时可按 `relay.retry_count` 换候选；默认 400/401/403 不重试；达到 `error_count` 阈值的通道会自动过滤并在冷却窗口后半开探测 | 定时探测、更多限流、更多故障注入和流式不可切换通道可测 |
