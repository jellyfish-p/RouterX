...
- Anthropic/Gemini 非流式入口已覆盖基础成功响应、usage、扣费和非文本 content/parts 的可解释降级；降级摘要会写入脱敏的 `request_snapshot.adapter_degradations`。
...
- Responses 已支持 OpenAI-compatible JSON/SSE 透传和 Azure `/openai/v1/responses?api-version=preview` JSON 转发，Responses SSE 会从 `response.completed.response.usage` 提取用量并扣费，仍需继续补完整原生事件矩阵和更多上游转换；Image Edits/Variations 已支持 OpenAI-compatible multipart 表单基础透传、`routerx` 表单字段或 `X-RouterX-Options` header 路由偏好、`relay.max_multipart_file_bytes` 单文件字段限制、路径形态和危险扩展名基础扫描、最低计费，其中 Azure Image Edits 已支持 `/openai/v1/images/edits?api-version=preview`；Azure Image Variations 端点尚未验证。仍需补内容安全扫描、字段保真和价格规则；Audio Transcriptions/Translations 已支持 OpenAI-compatible multipart 表单基础透传、`routerx` 表单字段或 `X-RouterX-Options` header 路由偏好、`relay.max_multipart_file_bytes` 单文件字段限制、路径形态和危险扩展名基础扫描、最低计费，仍需补完整音频格式策略、时长限制和内容安全扫描；Audio Speech 已支持 JSON 请求和二进制响应基础透传，但仍需补完整音频格式策略、时长限制和价格规则；Embeddings 已支持上游前 input schema 和 2048 批量边界校验，仍需补更细模型能力策略；Image Generations/Moderations 仍需补完整安全策略和价格规则。
...
read request body with relay.max_request_body_bytes
    -> validate multipart file fields with relay.max_multipart_file_bytes
    -> adapter.ConvertRequest
    -> http.Client.Do
...
- OpenAI-compatible multipart 文件名命中路径形态或危险扩展名基础扫描时，本地返回 400 `unsafe_multipart_file`，且不选择或调用上游、不扣费。
- 需要记录下游状态码。
...
- 请求体日志默认关闭。
- 图片、音频 multipart 文件字段受 `relay.max_multipart_file_bytes` 限制，并会拒绝路径形态文件名和明显可执行或脚本类危险扩展名。
- 对下游响应体设置最大读取大小。
