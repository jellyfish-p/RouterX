...
当前 `/ready` 挂在公共路由上，已检查数据库连接、外部数据库模式下 Redis 可用性、初始化后的 JWT 配置、关键 settings、支付 provider 环境密钥，以及存在 `enc:v1:` 通道密钥时的 `ENCRYPTION_KEY`。目标生产版本应继续检查迁移状态、KMS/密文逐条解密巡检和更多 Redis 集群策略；这些检查属于运维就绪，不应阻塞 `/health` 存活探测。
...
| Readiness | 已检查 DB、JWT、外部数据库 Redis、关键 settings、支付密钥和已有加密通道密钥的 `ENCRYPTION_KEY`；后续补迁移状态、KMS/逐条解密巡检和更多 Redis 策略 |
