...
| GET | `/metrics` | 基础实现 | Prometheus 文本指标；默认由 `observability.metrics_enabled=false` 关闭，已包含实例、Relay 日志、Relay 请求数、Relay 错误维度、通道可用状态、逐通道错误计数、限流拒绝、计费失败、支付、审计和 DB/Redis 基础指标 |
