...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/v1/responses` | 基础实现，OpenAI-compatible Responses JSON 或 SSE 透传；Azure OpenAI 通道转发到 `/openai/v1/responses?api-version=preview`；Claude/Anthropic 通道支持基础非流式 Responses 转 Messages，并将 Claude usage 映射回 Responses usage；usage 映射和扣费 |
