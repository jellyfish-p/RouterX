...
|------|------|----------|------|
| GET | `/health` | 已注册 | 健康检查 |
| GET | `/ready` | 已实现 | 就绪检查，检查数据库、外部数据库模式下 Redis 可用性、初始化后的 JWT 配置、关键 auth/relay/rate-limit settings 注册表校验、已启用支付 provider 的必需密钥，以及存在 `enc:v1:` 通道密钥时的 `ENCRYPTION_KEY` |
