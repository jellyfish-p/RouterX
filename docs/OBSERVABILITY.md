...
这些能力构成 P0 的可见闭环，并补上了 API Key 管理、用户管理、支付商品管理、系统/通道模型价格管理、支付入账/退款回调、Stripe/易支付 provider 退款请求、Stripe 争议生命周期、支付人工修正与人工退款、settings 变更、用户调额、充值码管理、通道管理、管理员账号管理、日志清理和日志导出审计的基础切片。商业级增强需要继续补更完整的审计覆盖、结构化字段、指标、告警、追踪和保留策略。
...
| `policy_snapshot` | 脱敏策略快照；当前包含成功 allow、额度预检、基础 scope allow、API Key scope 拒绝、基础余额预检拒绝、用户分组 x 通道分组访问控制拒绝、无可用候选 `no_available_channel` 拒绝，以及 Redis 全局/IP/Token/User/Model/Channel 限流拒绝摘要和 `rate_limit_snapshot` |
...
| P1 | 已补调用日志 request_id、error_code、usage_source、error_source、upstream_status、基础 request_snapshot、覆盖成功、API Key scope 拒绝、基础余额预检拒绝、用户分组访问控制拒绝、无可用候选拒绝和 Redis 全局/IP/Token/User/Model/Channel 限流拒绝分支的基础 policy_snapshot 与 `rate_limit_snapshot`、含过滤/模型重写/重试摘要的基础 route_snapshot 和含价格表达式或 P0 回退表达式/规则版本/业务倍率/预算前后摘要的基础 billing_snapshot；继续补更完整结构化失败事实。 |
