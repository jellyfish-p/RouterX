...
| `route_snapshot.filtered_reasons` / `route_snapshot.retry_attempts` | 候选过滤、模型重写和重试摘要的后续增强 |
...
| 诊断详情 | 单次调用能关联 request_id、error_code 和基础 route_snapshot；billing_snapshot 与完整路由过滤摘要仍需补齐。 |
...
| P1 | 已补调用日志 request_id、error_code、usage_source、error_source、upstream_status 和基础 route_snapshot；继续补 billing_snapshot、完整路由过滤摘要和更完整结构化失败事实。 |
