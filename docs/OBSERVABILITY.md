...
- `GET /metrics` 已注册为 Prometheus 文本指标接口，默认由 `observability.metrics_enabled=false` 关闭；启用后暴露用户数、API Key 数、通道数、可用通道数、当日调用/额度、ready、DB/Redis up、调用日志状态、总额度、逐通道可用状态、通道错误计数、限流拒绝、支付订单和支付事件指标。
...
| 指标 | `/metrics` 暴露基础实例、Relay 日志、通道可用状态、限流拒绝、支付和 DB/Redis 指标，不包含高基数或敏感 label；后续继续补 HTTP 耗时、上游耗时、更多错误维度和告警。 |
