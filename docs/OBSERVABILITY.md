...
当前 HTTP 中间件会按 `observability.request_id_header` 读取或生成请求 ID，并通过同名响应头返回；模型调用日志和管理审计已持久化 `request_id`；`/v1` 调用真实上游时会用当前配置的 request id header 透传同一个请求 ID。Recovery 系统错误日志会记录 `request_id`、method、path、client_ip、panic 类型和堆栈，但不会写入原始 panic 值；跨实例追踪仍需按目标规则继续补齐。
