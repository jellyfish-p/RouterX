...
| 指标 | `/metrics` 暴露基础实例、HTTP 请求量/耗时、Relay 日志、Relay 请求数、Relay 错误维度、token 用量、按模型/供应商/用户组的额度消耗、通道可用状态、逐通道错误计数、限流拒绝、计费失败、支付、审计和 DB/Redis 指标，不包含高基数或敏感 label；后续继续补上游耗时、更细错误维度和告警。 |
