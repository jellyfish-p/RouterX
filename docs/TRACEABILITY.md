...
| P1-C5 | 价格表达式 | 运营方能按模型、通道和用量规则定价。 | `BILLING`、`SNAPSHOTS`、`DATA_MODEL`、`SETTINGS` | `model_prices`、`channel_model_prices`、expression engine | 系统模型价格表、通道模型价格覆盖、规则版本、用户侧价格就绪状态和 `model_price.*`/`channel_model_price.*` 审计已覆盖；表达式热路径执行、计费快照和历史解释测试仍需补齐 |
