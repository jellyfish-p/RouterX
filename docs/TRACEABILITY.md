...
| P1-C6 | 访问控制 | 普通用户只能使用允许的通道、模型和分组。 | `POLICIES`、`SNAPSHOTS`、`SECURITY`、`BILLING`、`API_KEYS`、`API`、`RELAY` | channel access policy、user group policy、API Key scope | 用户分组 x 通道分组访问控制已由 `TestUserGroupChannelGroupAccessFiltersRelayCandidates` 覆盖；模型、APIType、通道分组、IP/CIDR、方法路径 allow-list 和日预算 scope 收窄已由 `TestAPIKeyModelScopeRestrictsRelayBeforeUpstream`、`TestAPIKeyAPIScopeRestrictsRelayBeforeUpstream`、`TestAPIKeyChannelGroupScopeFiltersRelayCandidates`、`TestAPIKeyIPScopeRejectsBeforeRelay`、`TestAPIKeyMethodScopeRejectsBeforeRelay`、`TestAPIKeyDailyQuotaScopeRejectsAfterDailyBudgetUsed` 覆盖；完整日志快照测试仍需补齐 |
...
| P2-C6 | 高级 API Key 管理 | 技术用户能批量管理、轮换和审计 API Key。 | `API_KEYS`、`SECURITY`、`API`、`ACCOUNTS`、`DATA_MODEL`、`OBSERVABILITY` | token metadata、batch API、usage filters、audit events | 基础生命周期审计、轮换、泄露上报、单 Key 用量摘要、管理员跨用户查询、批量禁用、模型/APIType/通道分组/IP/方法路径 allow-list scope 和日预算已覆盖；协议入口/月预算/并发 scope、批量过期、风险视图和缓存失效仍需补齐 |
