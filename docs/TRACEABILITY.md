...
| P2-C2 | 管理审计 | 关键管理行为可追溯。 | `OBSERVABILITY`、`OPERATIONS`、`DATA_MODEL`、`ACCOUNTS` | audit logs、admin service hooks | API Key 管理、用户管理、支付商品管理、支付入账/退款回调、settings 更新、用户调额、充值码管理、通道管理、管理员账号管理、日志清理审计、基础 `/metrics` 和超级管理员查询边界已覆盖；价格、日志导出、更多失败和拒绝操作审计测试仍需补齐 |
| P2-C3 | 支付与充值插件 | 在线充值、充值码、退款和人工修正可选接入，不影响基础运营路径。 | `PAYMENTS`、`SECURITY`、`BILLING`、`API`、`OPERATIONS` | payment products、orders、events、quota transactions、provider webhook | 充值码生成/导入/作废/兑换审计、管理员调额、支付商品管理及审计、支付订单创建审计、本地 pending 订单、Stripe webhook 入账审计、Stripe 全额退款和扣回审计、易支付异步通知审计和同步返回基础测试已覆盖；部分退款、争议和人工修正测试仍需补齐 |
