...
| P2-C6 | 高级 API Key 管理 | 技术用户能批量管理、轮换和审计 API Key。 | `API_KEYS`、`SECURITY`、`API`、`ACCOUNTS`、`DATA_MODEL`、`OBSERVABILITY` | token metadata、batch API、usage filters、audit events | 基础生命周期审计、轮换、泄露上报、单 Key 用量摘要、按 Key 过滤用户日志和账单聚合、最近使用来源摘要、管理员跨用户查询、按环境/团队/应用/标签过滤、脱敏 CSV 导出、批量禁用、批量过期、批量操作无筛选拒绝审计、基础风险视图、泄露风险基础轮换建议、模型/APIType/通道分组/入口协议/IP/方法路径 allow-list scope、日/月预算、并发上限和 RPM/TPM 已覆盖；基础 Redis 鉴权 lookup cache、缓存命中回源校验、禁用失效和 router Redis 兼容由 `TestValidateAndGetTokenResolvesFromRedisAuthCache`、`TestAPIKeyAuthCacheWarmsAndClearsOnDisable` 及批量禁用/过期 router 回归覆盖；单 Key 元数据过滤和脱敏导出由 `TestAPIKeyMetadataFiltersAndSanitizedExport` 覆盖；单 Key 泄露窗口分析由 `TestAPIKeyLeakWindowSummarizesRecentUse` 覆盖；泄露上报管理员告警收件箱和确认处理由 `TestAPIKeyLeakReportCreatesAdminAlert` 覆盖；Webhook 告警投递 outbox、列表、手动重放和脱敏 payload 由 `TestAPIKeyLeakAlertWebhookDeliveryReplay` 覆盖；API Key 鉴权、生命周期、最近使用年龄、有限额度、轮换和泄露指标由 `TestMetricsEndpointIncludesAPIKeyLifecycleAndRiskSignals` 覆盖；服务账号主体、邮件和 IM 外部告警推送渠道仍待补 |
