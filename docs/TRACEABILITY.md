...
| P1-C7 | 重试、限流和熔断 | 上游临时故障和本地频率保护能被隔离，错误原因可解释。 | `RELAY`、`OPERATIONS`、`SETTINGS`、`RUNBOOKS` | retry policy、rate limit counters、error counter、channel health state | 非流式安全重试、`relay.retry_on_status` 状态码白名单、Redis Token 限流、响应读取上限、error_count 自动熔断候选过滤和冷却窗口后的半开候选探测测试已覆盖；Redis Token 限流拒绝会写失败日志和基础 `policy_snapshot`；后台探测任务、更多限流维度和完整限流/熔断快照仍需补齐 |
