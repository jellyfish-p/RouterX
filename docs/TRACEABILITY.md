...
| P2-C3 | 支付与充值插件 | 在线充值、充值码、退款和人工修正可选接入，不影响基础运营路径。 | `PAYMENTS`、`SECURITY`、`BILLING`、`API`、`OPERATIONS` | payment products、orders、events、quota transactions、provider webhook | 充值码兑换和管理员调额的基础额度流水测试已覆盖；支付签名、金额、订单幂等、退款和审计测试仍需补齐 |
