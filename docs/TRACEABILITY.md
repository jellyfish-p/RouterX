...
| P1-C10 | 独立日志数据库 | 大量调用日志可独立备份清理，账单最小事实仍可恢复。 | `OPERATIONS`、`OBSERVABILITY`、`BILLING`、`DATA_MODEL` | `LOG_SQL_DSN`、LogService、billing outbox/minimal facts | `LOG_SQL_DSN` 初始化、主库事实保留、日志库副本写入、运行期写入失败降级、日志库列表读取和查询失败回退已由 `TestInitLogDBUsesConfiguredDSN`、`TestLogServiceWritesMainFactAndExternalLogDB`、`TestLogServiceFallsBackWhenExternalLogDBWriteFails`、`TestLogServiceListsFromExternalLogDBWhenConfigured`、`TestLogServiceListFallsBackToMainDBWhenExternalLogDBFails` 覆盖；outbox 异步补写、日志库健康指标和冷热归档仍需补齐 |
