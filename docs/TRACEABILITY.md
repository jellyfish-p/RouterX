...
| P2-C3 | 支付与充值插件 | 在线充值、充值码、退款和人工修正可选接入，不影响基础运营路径。 | `PAYMENTS`、`SECURITY`、`BILLING`、`API`、`OPERATIONS` | payment products、orders、events、quota transactions、provider webhook | 充值码生成/导入/作废、兑换、管理员调额、支付商品管理、本地 pending 订单、Stripe webhook、易支付异步通知和同步返回基础测试已覆盖；退款和审计测试仍需补齐 |
