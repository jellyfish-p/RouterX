...
| P0-C4 | 用户登录和权限 | 用户用 User JWT 管理自身资源，管理员管理系统资源。 | `ACCOUNTS`、`API`、`ARCHITECTURE` | auth middleware、user/admin routes、role checks | `TestAdminPrivilegeBoundaries`、`TestUserRegisterRespectsRegistrationSettings`、`TestUserLoginRespectsLoginMethodSettings` |
...
| P2-C2 | 管理审计 | 关键管理行为可追溯。 | `OBSERVABILITY`、`OPERATIONS`、`DATA_MODEL`、`ACCOUNTS` | audit logs、admin service hooks | API Key 管理、用户管理、用户分组管理、支付商品管理、系统模型价格管理、通道模型价格管理、支付入账/退款回调、Stripe/易支付 provider 退款请求、Stripe 争议生命周期、支付人工修正、settings 更新和校验拒绝、用户调额、充值码管理、通道管理、管理员账号管理、日志清理、日志导出审计、基础 `/metrics` 和超级管理员查询边界已覆盖；更多失败和拒绝操作审计测试仍需补齐 |
