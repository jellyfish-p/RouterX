...
| 门禁 | 必须证明 | 主要证据 |
|------|----------|----------|
| G0 文档一致 | 主设计稿、路线图、API、数据模型、Relay、计费、测试不互相冲突；已注册公开 API 必须出现在 Apifox 可导入文档中，且每个公开 operation 都有人类可读说明。 | `TestApifoxOpenAPICoversRegisteredRoutes`、`TestApifoxOpenAPIOperationsHaveHumanReadableDocs`、旧术语扫描、`git diff --check`、文档链接检查。 |
