...
| S6 | 路由或 scope 越权 | 调用方通过 `routerx.route` 强制使用无权通道、禁用通道或高价通道，或通过 API Key 调用 scope 未允许模型/APIType/通道分组。 | 先做后台硬性过滤、API Key scope 和访问控制，再应用偏好；拒绝原因写日志。 | `TestChannelRoutingConfigResolution`、`TestAPIKeyModelScopeRestrictsRelayBeforeUpstream`、`TestAPIKeyAPIScopeRestrictsRelayBeforeUpstream`、`TestAPIKeyChannelGroupScopeFiltersRelayCandidates`、`routerx.route` 合法/拒绝路径测试。 |
...
| 预检拒绝 | 无效 Key、禁用用户、禁用 API Key、余额不足、scope 未允许模型/APIType/通道分组、禁用通道不会调用上游。 |
