...
RouterX 是一个开源的 AI 模型网关，用于统一管理多个大模型厂商的 API 访问。项目提供 OpenAI-compatible 转发入口、用户与管理员体系、API Key 管理、渠道管理、额度控制、调用日志和基础用量统计。
...
- 管理员用户、管理员账号、渠道、日志、设置和看板接口。
- PostgreSQL、MySQL、SQLite 多数据库支持，启动时自动执行嵌入式迁移。
- Redis 可选接入，用于缓存和限流；Redis 不可用时后端仍可启动。
...
│   ├── dto/             # 请求/响应 DTO
│   ├── handler/         # HTTP 控制器
│   ├── middleware/      # CORS、鉴权、限流、恢复、日志等中间件
│   ├── migrate/         # 嵌入式 SQL 迁移
│   ├── model/           # GORM 数据模型
│   ├── relay/           # 上游模型厂商适配器
│   ├── router/          # 路由注册
...
| `CORS_ALLOWED_ORIGINS` | 空 | CORS 来源白名单，逗号分隔；优先于默认本地前端来源 |
...
RouterX 正处于快速迭代阶段。当前代码已经覆盖初始化、登录、JWT 鉴权、API Key、渠道管理、日志、设置、基础限流和 OpenAI-compatible Chat Completions 非流式转发；多协议完整转换、更多上游厂商适配、流式响应、在线支付和更细粒度计费仍在后续路线图中。
